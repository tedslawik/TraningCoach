...
function fmtPace100m(distKm: number, timeMin: number) {
  if (!distKm || !timeMin) return null;
  const p = timeMin / (distKm * 10); // min per 100m
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const maxHRSeen = rawSwims.map(a => (a.max_heartrate as number) ?? 0).reduce((m, v) => Math.max(m, v), 0);
  ...
  let totalDistKm = 0, totalTimeMin = 0, totalSufferScore = 0, longestKm = 0, totalHR = 0, hrCount = 0;

  const swims = rawSwims.map(a => {
    ...
    return {
      ...
      distanceM:  Math.round((a.distance as number) ?? 0),
      timeFormatted: fmtTime(timeMin),
      pace: fmtPace100m(distKm, timeMin),
  ...
  if (hrZones) {
    const hrSwims = swims.filter(s => s.hasHeartRate);
    const streamResults = await Promise.allSettled(
      hrSwims.map(s => fetch(`https://www.strava.com/api/v3/activities/${s.id}/streams?keys=heartrate,time&series_type=time&resolution=medium`, { headers }).then(r => r.ok ? r.json() as Promise<Array<{ type: string; data: number[] }>> : null))
    );
    streamResults.forEach((result, idx) => {
      ...
      if (hrStream && tmStream) hrSwims[idx].zoneTimes = calcZoneTimes(hrStream, tmStream, hrZones);
    });
  }

  const weeklyZoneTimes = swims.reduce<number[]>((acc, s) => {
    if (!s.zoneTimes) return acc;
    s.zoneTimes.forEach((sec, i) => { acc[i] = (acc[i] ?? 0) + sec; });
    return acc;
  }, [0, 0, 0, 0, 0]);
  ...
  res.json({
    ...
    totals: {
      ...
      avgPace:       fmtPace100m(totalDistKm, totalTimeMin),
