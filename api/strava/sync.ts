...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    if (!process.env.CRON_SECRET || secret !== process.env.CRON_SECRET) return res.status(401).end();
...
async function syncUser(tokenRow: Record<string, unknown>): Promise<{ weeks: number; activities: number }> {
  const token = await refreshToken(tokenRow);
  const headers = { Authorization: `Bearer ${token}` };

  const athleteRaw = await fetch('https://www.strava.com/api/v3/athlete', { headers }).then(r => r.ok ? r.json() : {});
  const ftp: number | null = (athleteRaw?.ftp && athleteRaw.ftp > 0) ? athleteRaw.ftp : null;

  const after = Math.floor((Date.now() - 16 * 7 * 24 * 60 * 60 * 1000) / 1000);
  let page = 1;
  const allActivities: Record<string, unknown>[] = [];
  while (true) {
    const r = await fetch(`https://www.strava.com/api/v3/athlete/activities?after=${after}&per_page=200&page=${page}`, { headers });
  ...
  const byWeek: Record<string, { swimDistKm:number;swimTimeMin:number;swimSessions:number;bikeDistKm:number;bikeTimeMin:number;bikeSessions:number;runDistKm:number;runTimeMin:number;runSessions:number;sufferScore:number;tss:number;kilojoules:number }> = {};
  for (const a of allActivities) {
    const sport = a.sport_type as string;
    const date  = new Date((a.start_date_local as string) || (a.start_date as string));
    ...
    if (!byWeek[key]) byWeek[key] = { swimDistKm:0,swimTimeMin:0,swimSessions:0,bikeDistKm:0,bikeTimeMin:0,bikeSessions:0,runDistKm:0,runTimeMin:0,runSessions:0,sufferScore:0,tss:0,kilojoules:0 };
    const w = byWeek[key];
    w.sufferScore += suffer; w.tss += tss; w.kilojoules += kj;
    if      (SWIM.has(sport)) { w.swimDistKm += distKm; w.swimTimeMin += timeMin; w.swimSessions++; }
    else if (BIKE.has(sport)) { w.bikeDistKm += distKm; w.bikeTimeMin += timeMin; w.bikeSessions++; }
    else if (RUN.has(sport))  { w.runDistKm  += distKm; w.runTimeMin  += timeMin; w.runSessions++;  }
  }
  ...
  const rows = Object.entries(byWeek).map(([week_start, w]) => ({
    user_id: tokenRow.user_id, week_start,
    swim_dist_km: Math.round(w.swimDistKm*10)/10, swim_time_min: Math.round(w.swimTimeMin), swim_sessions: w.swimSessions,
    bike_dist_km: Math.round(w.bikeDistKm*10)/10, bike_time_min: Math.round(w.bikeTimeMin), bike_sessions: w.bikeSessions,
    run_dist_km:  Math.round(w.runDistKm *10)/10, run_time_min:  Math.round(w.runTimeMin),  run_sessions:  w.runSessions,
    suffer_score: Math.round(w.sufferScore), tss: Math.round(w.tss), kilojoules: Math.round(w.kilojoules),
    synced_at: new Date().toISOString(),
  }));

  await supabase.from('weekly_summaries').upsert(rows, { onConflict: 'user_id,week_start' });
  return { weeks: rows.length, activities: allActivities.length };
}

async function handleCron(res: VercelResponse) {
  const { data: tokens } = await supabase.from('strava_tokens').select('*');
  if (!tokens?.length) return res.json({ cron: true, users: 0 });

  const results = await Promise.allSettled(tokens.map(syncUser));
  const ok  = results.filter(r => r.status === 'fulfilled').length;
  const err = results.filter(r => r.status === 'rejected').length;
  return res.json({ cron: true, users: tokens.length, ok, err });
}
